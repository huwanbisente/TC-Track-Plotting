...
#    ax.gridlines()
    ...
    world, transform = get_geometries('featurecla', 'Admin-0 country', ax)
    for c in world:
        ax.add_geometries([c], ccrs.PlateCarree(), facecolor='white',
                          edgecolor='black', linewidth=0.5)
    ...
    for idx, tctrack in enumerate(tctracks):
        ...
        plt.text(-0.28, 0.99 - 0.035*idx, str(idx+1), fontsize=15, va='top', ha='right',
        ...
        plt.text(-0.27, 0.99 - 0.035*idx, tctrack.name + ' ' +
                 tctrack.start.strftime('%d/%b') + ' - ' +
                 tctrack.end.strftime('%d/%b'),
                 fontsize=15, va='top', color=track_color,
                 bbox={'facecolor':'none', 'edgecolor':'none','pad':5},
                 transform=ax.transAxes)


        ax.add_geometries([track], ccrs.PlateCarree(), facecolor='none',
                          edgecolor=track_color, linewidth=1.0)
...
#                             transform=ccrs.Geodetic())
                if is_inside(lon, lat, 90, 143, -15, 35):
                    if idx != 15:
                        plt.text(lon, lat, str(idx+1), fontsize=10, bbox={'facecolor':'white', 'pad':3},
                                 transform=ccrs.Geodetic())
                    else:
                        plt.text(lon, lat-1, str(idx+1), fontsize=10, bbox={'facecolor':'white', 'pad':3},
                                 transform=ccrs.Geodetic())
    ...
    plt.text(0.0225, 0.975, 'Tropical Cyclone Tracks July - Dec 2017',
             fontsize=15, va='top',
             bbox={'facecolor':'white', 'pad':5},
             transform=ax.transAxes)

    plt.text(0.85, 0.035, 'PLOT: PAGASA\nDATA: JMA & JTWC',
             fontsize=10, va='bottom',
             bbox={'facecolor':'white', 'pad':3},
             transform=ax.transAxes)


    plt.savefig(''.join(('2017tracks', '.png')), bbox_inches='tight')
